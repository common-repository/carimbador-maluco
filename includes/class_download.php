...
/**
* aqui nós fomos sapecas e usamos a classe de download do woocommerce
* mas fizemos isso porque avita incompatibilidade com nosso amiguinho
*/
class Carimbador_Download_Handler {
	...
	public static function init() {
		...
		remove_action('woocommerce_download_file_force',array( "WC_Download_Handler", 'download_file_force' ));
		add_action( 'woocommerce_download_file_force', array( __CLASS__, 'download_file_force' ), 10, 2 );
		add_action( 'woocommerce_download_file_preview', array( __CLASS__, 'download_file_preview' ), 10, 3 );
	...
	public static function buscaOrderId($chave){
		global $wpdb;
		$results = $wpdb->get_results( "SELECT order_id FROM {$wpdb->prefix}woocommerce_downloadable_product_permissions WHERE order_key = '$chave'", OBJECT );
  ...
  /**
	 ...
	 * @param  string $filename
	 */
	public static function download_file_preview( $file_path, $filename, $dados ) {
	  ...
	  if(!file_exists($parsed_file_path['file_path'])){
     $erro = "Impossivel Vizualizar este arquivo - Arquivo não encontrado";
      exit;
    }
		// se tivermos um PDF, vamos carimba-lo apropriadamente
		if(self::get_download_content_type( $parsed_file_path['file_path'] )=="application/pdf"){
			// se for um arquivo pdf remoto fazemos isso
			if ( $parsed_file_path['remote_file'] ) {
					$erro =  "Impossivel Vizualizar este arquivo - Arquivos remotos não podem ser vizualizados";
			}


			do_action("carimbador_vizualiza_arquivo",$parsed_file_path['file_path'],$filename,true, $dados); // chmamamos a função que faz a magica do Carimbador Maluco
				
			exit;	
		} else {
			// caso não seja um PDF, vamos forçar um download comum
			$erro =  "Impossivel Vizualizar este arquivo - Formato Incompatível ";
			}
	...
	public static function download_file_force( $file_path, $filename ) {
		...
		if(self::get_download_content_type( $parsed_file_path['file_path'] )=="application/pdf"){
			...
			//não era remoto e temos que trabalhar nele
			$dados = array($_GET['email'],self::buscaOrderId($_GET['order']));

			do_action("carimbador_salva_arquivo",$parsed_file_path['file_path'],$filename,$dados); // chmamamos a função que faz a magica do Carimbador Maluco
...
Carimbador_Download_Handler::init();